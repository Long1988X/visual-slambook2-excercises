...
string image1 = "../../images/LK1.png";
string image2 = "../../images/LK2.png";

int main(int argc, char** argv)
{
    // read images
    Mat img1 = imread(image1, 0);
    Mat img2 = imread(image2, 0);

    // detect keypoints of image1
    ...
    cout << "GFTT keypoints time cost: " << time_used.count() << " " << "keypoints nums:" << kp1.size() << endl;

    // use single level LK to track the keypoints in the second image
    ...
    cout << "Single level time cost: " << time_used.count() << endl;
    ...
    t2 = chrono::steady_clock::now();
    time_used = chrono::duration_cast<chrono::duration<double>>(t2 - t1);
    cout << "Multi level time cost: " << time_used.count() << endl;
    ...
    for (int i = 0; i < pyramids; i++) {
        if (i == 0) {
            ...
            cv::resize(pyr1[i - 1], img1_pyr, 
                        cv::Size(pyr1[i-1].cols * pyramid_scale, pyr1[i-1].cols * pyramid_scale));
            cv::resize(pyr2[i - 1], img2_pyr, 
                        cv::Size(pyr2[i-1].cols * pyramid_scale, pyr2[i-1].cols * pyramid_scale));
